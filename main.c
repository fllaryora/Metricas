...
int main(int argc, char **argv){
	...
	const char *filenameJson = "input/input.json";
	...
	//benchmark code
	if(idNodo == MASTER_ID){
		benckmarkCsv = open ("output/benchmark.csv" , O_WRONLY|O_CREAT|O_APPEND,00660);
...
//void master(const int mpiProcesses, const MPI_Comm commNodes ,const char *filenameJson ){
	...
	if ( vr->isValid == VALIDATION_PASS ) {		
		if ( vr->nodesAmount + MASTER_RAFFLER_PRINTER == mpiProcesses ) {
			...
			int fileDescriptor = open ("output/output.json" , O_WRONLY|O_CREAT|O_TRUNC,00660);
